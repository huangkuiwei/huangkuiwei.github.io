$(document).ready(function () {
  var platformData = {
    ...
    platform: {}, // 导航栏选择的平台
    mode: 1, // 1:最新 2:最热
    platformListDom: $('.platform-page .platform-list'),
    selectModeDom: $('.platform-page .router .type span'),
    platformListInnerHTML: '',

    // 加载 loading 动画
    loading: function () {
      // 移除前一次动画
      var originalLoadingDom = document.querySelector('.platform-page .platform-list .custom-loading');
      var parent = document.querySelector('.platform-page .platform-list');
    ...
    renderPlatformList: function () {
      ...
      platformData.platformList.forEach(function (platform) {
        ...
        if (platform.isSubscribe) {
          ...
          platformData.platformListInnerHTML += '<span class="name pointer">订阅</span>';
        }
        ...
        if (platform.isSubscribe) {
          ...
          platformData.platformListInnerHTML +=
            '<img class="icon pointer" src="./static/images/platform/subscribe.png" alt="" />';
        }
    ...
    queryPlatform: function (data) {
      platformData.platform = data && data.platform;
    ...
    changeMode: function (event) {
      platformData.mode = event.target.dataset.mode;
      platformData.selectModeDom.removeClass('active');

      platformData.selectModeDom.each(function (index) {
        if (this.dataset.mode === platformData.mode) {
  ...
  $(document).on('platformChange', function (event, data) {
    platformData.queryPlatform(data);
  });

  platformData.selectModeDom.on('click', function (event) {
    platformData.changeMode(event);
  });
});
